...
{
    public class RedisRedisCache : IRedisCache
    {
        private readonly ILogger<RedisRedisCache> _logger;

        public RedisRedisCache(ILogger<RedisRedisCache> logger)
        {
            _logger = logger;
        }
        ...
        /// <summary>为哈希表 key 中的指定字段的整数值加上增量 increment</summary>
        /// <param name="key">不含prefix前辍</param>
        /// <param name="field">字段</param>
        /// <param name="value">增量值(默认=1)</param>
        /// <returns></returns>
        public async Task<long> HIncrByAsync(string key, string field, long value = 1)
        {
            return await RedisHelper.HIncrByAsync(key, field, value);
        }
        ...
        {
            List<object> objectarr = new List<object>(0);
